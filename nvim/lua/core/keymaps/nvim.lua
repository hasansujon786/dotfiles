...
local maps_common = {
  ...
  { "n", "nzz", remap = true, desc = "Repeat the latest \"/\" or \"?\"", mode = { "n", "x" } },
  { "N", "Nzz", remap = true, desc = "Repeat the latest \"/\" or \"?\"", mode = { "n", "x" } },
  { "'", "`", remap = true, desc = "Jump to the mark", mode = { "n", "x" } },
  { "@", ":norm @", desc = "Run macro on visual selection", mode = "v", silent = false },
  { "p", "pgvy", mode = "v" },
...
local maps = {
  ...
  { "<C-v>", "<C-R>+", desc = "Paste from system clipboard", mode = { "i", "c" }, silent = false },
  { "<C-g><C-v>", "<C-v>", desc = "Paste from system clipboard", mode = { "i", "c" }, silent = false },
  { "<A-p>", "<C-R>\"", desc = "Paste the last item from register", mode = { "i", "c" }, silent = false },
  { "<A-k>", "<esc><cmd>m .-2<cr>==gi", desc = "Move Up", mode = "i" },
  { "<A-j>", "<esc><cmd>m .+1<cr>==gi", desc = "Move Down", mode = "i" },
  { "<C-_>", "mz_gcc`z", remap = true, desc = "Toggle comment line", mode = "n" },
  { "<C-_>", "<ESC>_gccgi", remap = true, desc = "Toggle comment line", mode = "i" },
  { "<C-_>", "mz_gcgv`z", remap = true, desc = "Toggle comment line", mode = "v" },
  { "zuu", "0vai:foldclose!<CR>zazt", remap = true, desc = "Fold current context", mode = { "n", "x" } },
  { "zu", ":foldclose!<CR>zazt", remap = true, desc = "Fold current context", mode = { "n", "x" } },
  ...
  { "k", "v:count == 0 ? \"gk\" : \"k\"", remap = false, desc = "Move cursor up", expr = true },
  { "j", "v:count == 0 ? \"gj\" : \"j\"", remap = false, desc = "Move cursor down", expr = true },
  { "<A-u>", "<C-u>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-d>", "<C-d>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-o>", "<C-d>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<PageUp>", "<C-u>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<PageDown>", "<C-d>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-f>", "<C-f>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-b>", "<C-b>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-y>", "<C-y>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-e>", "<C-e>", remap = true, desc = "Scroll window", mode = { "n", "x" } },
  { "<A-h>", "20zh", mode = { "n", "x" } },
  { "<A-l>", "20zl", mode = { "n", "x" } },
