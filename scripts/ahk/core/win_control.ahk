...
ToggleApp(exeName, startProgram := "", excludeClass := "") {
	if !ProcessExist(exeName) {
		...
		try {
			...
			; Briefly activate the previous window then back to the app
			; This "inserts" the previous window into Alt+Tab history
			if (previousWindow) {
				Sleep(100)  ; Small delay for app to fully activate
...
ToggleExplorer() {
	if WinExist("ahk_class CabinetWClass") {
		if WinActive("ahk_class CabinetWClass") {
			...
			; Explorer is not in focus, activate it
			WinActivate("ahk_class CabinetWClass")
		}
	} else {
		; No Explorer window exists, open one
		Run("explorer.exe")
